export function getBreadboardInternalConnections(pinName: string): string[] {
  // Power rails
  if (pinName.startsWith("tp.")) return Array.from({ length: 25 }, (_, i) => `tp.${i}`);
  if (pinName.startsWith("tg.")) return Array.from({ length: 25 }, (_, i) => `tg.${i}`);
  if (pinName.startsWith("bp.")) return Array.from({ length: 25 }, (_, i) => `bp.${i}`);
  if (pinName.startsWith("bg.")) return Array.from({ length: 25 }, (_, i) => `bg.${i}`);
  ...
  if (match) {
    ...
    const group = colIndex < 5 ? ['a', 'b', 'c', 'd', 'e'] : ['f', 'g', 'h', 'i', 'j'];
